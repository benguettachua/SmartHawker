...
class EditProfileViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        view.layout(name).top(170).horizontally(left: 20, right: 20).height(22)
        
        let phone = TextField()
        phone.placeholder = "PHONE"
        phone.text = "98765432"
        
        view.layout(phone).top(215).horizontally(left: 20, right: 20).height(22)
        
        let email = TextField()
        email.placeholder = "EMAIL"
        email.text = "abc@sis"

        view.layout(email).top(260).horizontally(left: 20, right: 20).height(22)
    ...
    view.layout(bizname).top(305).horizontally(left: 20, right: 20).height(22)
        
        let biznum = TextField()
        biznum.placeholder = "BUSINESS NUMBER"
        biznum.text = "12345678"

        view.layout(biznum).top(350).horizontally(left: 20, right: 20).height(22)
        
        let address = TextField()
        address.placeholder = "ADDRESS"
        address.text = "21 seletar road"

    view.layout(address).top(395).horizontally(left: 20, right: 20).height(22)
    }
